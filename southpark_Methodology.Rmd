...
```{r breakdown}
counts <- data.frame(grams=c("unigrams", "bigrams", "trigrams", "4-grams", "5-grams"),
                     unique=c(2379, 5849, 1927, 742, 131),
                     total=c(172269, 168320, 31027, 6911, 1610))
kable(counts, row.names=FALSE)
```

```{r}
setwd("/Users/kaylinwalker/Desktop/South Park")
#source("southpark_Scrape_2.R") # scrapes raw text from the web, saves into 'all', 'byepisode' and 'byperson' files
#source("southpark_getCorpus.R") # creates a corpus of the byperson file
#source("southpark_LL.R") # generates log likelihoods for each corpus of words

setwd("/Users/kaylinwalker/Desktop/South Park/data")
